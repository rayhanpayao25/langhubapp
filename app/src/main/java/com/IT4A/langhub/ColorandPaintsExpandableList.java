...
public class ColorandPaintsExpandableList extends BaseExpandableListAdapter {

    private Context context;
    private List<String> colorandpaints;
    private Map<String, List<String>> translationsMap;
    private List<String> filteredColorandpaints;

    public ColorandPaintsExpandableList(Context context, List<String> colorandpaints, Map<String, List<String>> translationsMap) {
        this.context = context;
        this.colorandpaints = colorandpaints;
        this.translationsMap = translationsMap;
        this.filteredColorandpaints = colorandpaints; // Start with the full list
    }

    @Override
    public int getGroupCount() {
        return filteredColorandpaints.size();
    }
    ...
    public Object getGroup(int groupPosition) {
        return filteredColorandpaints.get(groupPosition);
    }
    ...
    public View getGroupView(int groupPosition, boolean isExpanded, View convertView, ViewGroup parent) {
        ...
        title.setText(filteredColorandpaints.get(groupPosition));
        ...
        String spanishTranslation = translationsMap.get(filteredColorandpaints.get(groupPosition)).get(0).replace("Spanish: ", "");
        String mandarinTranslation = translationsMap.get(filteredColorandpaints.get(groupPosition)).get(1).replace("Mandarin: ", "");
        ...
        spanishSpeakerIcon.setOnClickListener(v -> {
            ...
            String spanishTextToSpeak = translationsMap.get(filteredColorandpaints.get(groupPosition)).get(0);
            if (context instanceof ColorandPaintsActivity) {
                ((ColorandPaintsActivity) context).speakText(spanishTextToSpeak);
            }
        });
        ...
        mandarinSpeakerIcon.setOnClickListener(v -> {
            ...
            String mandarinTextToSpeak = translationsMap.get(filteredColorandpaints.get(groupPosition)).get(1);
            if (context instanceof ColorandPaintsActivity) {
                ((ColorandPaintsActivity) context).speakText(mandarinTextToSpeak);
            }
        });
    ...
    public void filterList(String query) {
        if (query.isEmpty()) {
            filteredColorandpaints = colorandpaints;  // Return to the full list if query is empty
        } else {
            List<String> filteredList = new ArrayList<>();

            for (String item : colorandpaints) {
            ...
            filteredColorandpaints = filteredList;  // Set filtered list
        }
