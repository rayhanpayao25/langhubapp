...
public class EatingoutExpandableList extends BaseExpandableListAdapter {

    private Context context;
    private List<String> eatingoutList;
    private Map<String, List<String>> translationsMap;
    private List<String> filteredEatingoutList;

    public EatingoutExpandableList(Context context, List<String> eatingoutList, Map<String, List<String>> translationsMap) {
        this.context = context;
        this.eatingoutList = eatingoutList;
        this.translationsMap = translationsMap;
        this.filteredEatingoutList = eatingoutList; // Start with the full list
    }

    @Override
    public int getGroupCount() {
        return filteredEatingoutList.size();
    }
    ...
    public Object getGroup(int groupPosition) {
        return filteredEatingoutList.get(groupPosition);
    }
    ...
    public View getGroupView(int groupPosition, boolean isExpanded, View convertView, ViewGroup parent) {
        ...
        title.setText(filteredEatingoutList.get(groupPosition));
        ...
        String spanishTranslation = translationsMap.get(filteredEatingoutList.get(groupPosition)).get(0).replace("Spanish: ", "");
        String mandarinTranslation = translationsMap.get(filteredEatingoutList.get(groupPosition)).get(1).replace("Mandarin: ", "");
        ...
        spanishSpeakerIcon.setOnClickListener(v -> {
            ...
            String spanishTextToSpeak = translationsMap.get(filteredEatingoutList.get(groupPosition)).get(0);
            if (context instanceof EatingoutActivity) {
                ((EatingoutActivity) context).speakText(spanishTextToSpeak);
            }
        });
        ...
        mandarinSpeakerIcon.setOnClickListener(v -> {
            ...
            String mandarinTextToSpeak = translationsMap.get(filteredEatingoutList.get(groupPosition)).get(1);
            if (context instanceof EatingoutActivity) {
                ((EatingoutActivity) context).speakText(mandarinTextToSpeak);
            }
        });
    ...
    public void filterList(String query) {
        if (query.isEmpty()) {
            filteredEatingoutList = eatingoutList;  // Return to the full list if query is empty
        } else {
            List<String> filteredList = new ArrayList<>();

            for (String item : eatingoutList) {
            ...
            filteredEatingoutList = filteredList;  // Set filtered list
        }
