...
public class ConversationExpandableListAdapter extends BaseExpandableListAdapter {

    private Context context;
    private List<String> conversationList;
    private Map<String, List<String>> translationsMap;
    private List<String> filteredConversationList;

    public ConversationExpandableListAdapter(Context context, List<String> conversationList, Map<String, List<String>> translationsMap) {
        this.context = context;
        this.conversationList = conversationList;
        this.translationsMap = translationsMap;
        this.filteredConversationList = conversationList; // Start with the full list
    }

    @Override
    public int getGroupCount() {
        return filteredConversationList.size();
    }
    ...
    public Object getGroup(int groupPosition) {
        return filteredConversationList.get(groupPosition);  // Return the correct conversation from the filtered list
    }
    ...
    public View getGroupView(int groupPosition, boolean isExpanded, View convertView, ViewGroup parent) {
        ...
        title.setText(filteredConversationList.get(groupPosition));
        ...
        String spanishTranslation = translationsMap.get(filteredConversationList.get(groupPosition)).get(0).replace("Spanish: ", "");
        String mandarinTranslation = translationsMap.get(filteredConversationList.get(groupPosition)).get(1).replace("Mandarin: ", "");
        ...
        spanishSpeakerIcon.setOnClickListener(v -> {
            ...
            String spanishTextToSpeak = translationsMap.get(filteredConversationList.get(groupPosition)).get(0);
            if (context instanceof ConversationActivity) {
                ((ConversationActivity) context).speakText(spanishTextToSpeak);
            }
        });
        ...
        mandarinSpeakerIcon.setOnClickListener(v -> {
            ...
            String mandarinTextToSpeak = translationsMap.get(filteredConversationList.get(groupPosition)).get(1);
            if (context instanceof ConversationActivity) {
                ((ConversationActivity) context).speakText(mandarinTextToSpeak);
            }
        });
    ...
    public void filterList(String query) {
        if (query.isEmpty()) {
            filteredConversationList = conversationList;  // Return to the full list if query is empty
        } else {
            List<String> filteredList = new ArrayList<>();

            for (String conversation : conversationList) {
                if (conversation.toLowerCase().contains(query.toLowerCase()) ||
                        translationsMap.get(conversation).get(0).toLowerCase().contains(query.toLowerCase()) ||  // Check Spanish translation
                        translationsMap.get(conversation).get(1).toLowerCase().contains(query.toLowerCase())) {  // Check Mandarin translation
                    filteredList.add(conversation);  // Add conversations that match the query
                }
            }
            filteredConversationList = filteredList;  // Set filtered list
        }
