...
public class TimeanddateExpandableListAdapter extends BaseExpandableListAdapter {

    private Context context;
    private List<String> timeAndDateList;
    private Map<String, List<String>> translationsMap;
    private List<String> filteredTimeAndDateList;

    public TimeanddateExpandableListAdapter(Context context, List<String> timeAndDateList, Map<String, List<String>> translationsMap) {
        this.context = context;
        this.timeAndDateList = timeAndDateList;
        this.translationsMap = translationsMap;
        this.filteredTimeAndDateList = timeAndDateList; // Start with the full list
    }

    @Override
    public int getGroupCount() {
        return filteredTimeAndDateList.size();
    }
    ...
    public Object getGroup(int groupPosition) {
        return filteredTimeAndDateList.get(groupPosition);  // Return the correct time/date item from the filtered list
    }
    ...
    public View getGroupView(int groupPosition, boolean isExpanded, View convertView, ViewGroup parent) {
        ...
        title.setText(filteredTimeAndDateList.get(groupPosition));
        ...
        String spanishTranslation = translationsMap.get(filteredTimeAndDateList.get(groupPosition)).get(0).replace("Spanish: ", "");
        String mandarinTranslation = translationsMap.get(filteredTimeAndDateList.get(groupPosition)).get(1).replace("Mandarin: ", "");
        ...
        spanishSpeakerIcon.setOnClickListener(v -> {
            ...
            String spanishTextToSpeak = translationsMap.get(filteredTimeAndDateList.get(groupPosition)).get(0);
            if (context instanceof TimeanddateActivity) {
                ((TimeanddateActivity) context).speakText(spanishTextToSpeak);
            }
        });
        ...
        mandarinSpeakerIcon.setOnClickListener(v -> {
            ...
            String mandarinTextToSpeak = translationsMap.get(filteredTimeAndDateList.get(groupPosition)).get(1);
            if (context instanceof TimeanddateActivity) {
                ((TimeanddateActivity) context).speakText(mandarinTextToSpeak);
            }
        });
    ...
    public void filterList(String query) {
        if (query.isEmpty()) {
            filteredTimeAndDateList = timeAndDateList;  // Return to the full list if query is empty
        } else {
            List<String> filteredList = new ArrayList<>();

            for (String item : timeAndDateList) {
            ...
            filteredTimeAndDateList = filteredList;  // Set filtered list
        }
