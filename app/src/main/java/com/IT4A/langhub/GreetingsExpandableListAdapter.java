...
public class GreetingsExpandableListAdapter extends BaseExpandableListAdapter {

    private Context context;
    private List<String> greetingList;
    private Map<String, List<String>> translationsMap;
    private List<String> filteredGreetingList;

    public GreetingsExpandableListAdapter(Context context, List<String> greetingList, Map<String, List<String>> translationsMap) {
        this.context = context;
        this.greetingList = greetingList;
        this.translationsMap = translationsMap;
        this.filteredGreetingList = greetingList; // Start with the full list
    }

    @Override
    public int getGroupCount() {
        return filteredGreetingList.size();
    }
    ...
    public Object getGroup(int groupPosition) {
        return filteredGreetingList.get(groupPosition);
    }
    ...
    public View getGroupView(int groupPosition, boolean isExpanded, View convertView, ViewGroup parent) {
        ...
        title.setText(filteredGreetingList.get(groupPosition));
        ...
        String spanishTranslation = translationsMap.get(filteredGreetingList.get(groupPosition)).get(0).replace("Spanish: ", "");
        String mandarinTranslation = translationsMap.get(filteredGreetingList.get(groupPosition)).get(1).replace("Mandarin: ", "");
        ...
        spanishSpeakerIcon.setOnClickListener(v -> {
            ...
            String spanishTextToSpeak = translationsMap.get(filteredGreetingList.get(groupPosition)).get(0);
            if (context instanceof GreetingsActivity) {
                ((GreetingsActivity) context).speakText(spanishTextToSpeak);
            }
        });
        ...
        mandarinSpeakerIcon.setOnClickListener(v -> {
            ...
            String mandarinTextToSpeak = translationsMap.get(filteredGreetingList.get(groupPosition)).get(1);
            if (context instanceof GreetingsActivity) {
                ((GreetingsActivity) context).speakText(mandarinTextToSpeak);
            }
        });
    ...
    public void filterList(String query) {
        if (query.isEmpty()) {
            filteredGreetingList = greetingList;  // Return to the full list if query is empty
        } else {
            List<String> filteredList = new ArrayList<>();

            for (String greeting : greetingList) {
                if (greeting.toLowerCase().contains(query.toLowerCase()) ||
                        translationsMap.get(greeting).get(0).toLowerCase().contains(query.toLowerCase()) ||  // Check Spanish translation
                        translationsMap.get(greeting).get(1).toLowerCase().contains(query.toLowerCase())) {  // Check Mandarin translation
                    filteredList.add(greeting);
                }
            }
            filteredGreetingList = filteredList;  // Set filtered list
        }
